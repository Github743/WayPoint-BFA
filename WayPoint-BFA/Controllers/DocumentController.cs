...
namespace WayPoint_BFA.Controllers
{
    ...
    public class DocumentController(IDocuments document) : ControllerBase
    {
        ...
        {
            bool isLiscr = true;
            var lOutgoingDocuments = await _document.GetWorkOrderFiles(workOrderId, 0,false,false, isLiscr, ct);
            ...
            return [.. lOutgoingDocuments.OrderByDescending(x => x.DefaultOrder.HasValue).ThenBy(x => x.DefaultOrder).ThenBy(x => x.DisplayName)]; 
        }
