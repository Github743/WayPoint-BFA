...
using BFAWebApp.Infrastructure.StoredProcedures;
using Microsoft.Data.SqlClient;
using System.Data;

namespace BFAWebApp.Infrastructure.Repositories;

public class SystemDiscountRepository(ISqlRunner sql) : ISystemDiscountRepository
{
    private readonly ISqlRunner _sql = sql;

    public async Task<IReadOnlyList<SystemDiscountSchedule>> GetDiscountSchedulesAsync(
        int workOrderId, int systemDiscountProgramId, CancellationToken ct = default)
    {
        ...
        var agreements = await _sql.QueryProcAsync<WorkOrderClientAgreement, Domain.ReadModels.WorkOrderClientAgreementDto>(
            SqlVerb.Get,
        ...
        var schedules = await _sql.QueryProcAsync<SystemDiscountSchedules, SystemDiscountSchedule>(
            SqlVerb.Get,
            parameters: schedulesParams,
            map: r => new SystemDiscountSchedule
            {
